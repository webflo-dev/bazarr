...
class Addic7edProvider(_Addic7edProvider):
    ...
    def initialize(self):
        ...
        if self.username is not None and self.password is not None:
            ...
            self.cookies = dict(wikisubtitlesuser=self.username, wikisubtitlespass=self.password)
            r = self.session.get(self.server_url + 'shows.php', cookies=self.cookies, allow_redirects=True, timeout=10)
            soup = ParserBeautifulSoup(r.content, ['lxml', 'html.parser'])
            btn = soup.select('a[href="/logout.php"]')
            if not btn:
                raise AuthenticationError(self.username)
    ...
    def _get_show_ids(self):
        ...
        r = self.session.get(self.server_url + 'shows.php', timeout=10, cookies=self.cookies)
        r.raise_for_status()
    ...
    def _search_show_id(self, series, year=None):
        ...
        for endpoint in ("srch.php", "search.php",):
            ...
            r = self.session.get(self.server_url + endpoint, params=params, timeout=10, headers=headers, cookies=self.cookies)
            r.raise_for_status()
    ...
    def download_subtitle(self, subtitle):
        ...
        r = self.session.get(self.server_url + subtitle.download_link, headers={'Referer': subtitle.page_link},
                             timeout=10, cookies=self.cookies)
        r.raise_for_status()
